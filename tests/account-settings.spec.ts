...
test.describe("Página de Configurações de Conta", () => {
  ...
  test("deve alternar entre as abas", async ({ page }) => {
    ...
    await expect(
      page.getByRole("tab", { name: /Notificações/i }),
    ...
    await expect(page.getByRole("tab", { name: /Segurança/i })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  ...
  test("deve mostrar o formulário de informações do cliente na aba Conta", async ({
    page,
  }) => {
  ...
  test("deve alternar entre pessoa física e jurídica", async ({ page }) => {
    ...
    await expect(
      page.getByText("Inscrição Estadual", { exact: false }),
    ).toBeVisible();
  ...
  test("deve mostrar as opções de endereço de cobrança", async ({ page }) => {
    ...
    const checkboxLabel = page.getByText(
      "Endereço de cobrança diferente do endereço principal",
    );

...
test.describe('Interações na Página de Configurações de Conta', () => {
  ...
  test('deve validar campos obrigatórios no formulário de conta', async ({ page }) => {
  ...
  test('deve formatar CPF automaticamente', async ({ page }) => {
    ...
    await expect(page.getByLabel('CPF', { exact: false })).toHaveValue('123.456.789-00');
  });
  ...
  test('deve formatar Celular automaticamente', async ({ page }) => {
    ...
    await expect(page.getByLabel('Celular', { exact: false })).toHaveValue('(11) 98765-4321');
  });

  test('deve mostrar campos de pessoa jurídica e validar CNPJ', async ({ page }) => {
    ...
    await expect(page.getByLabel('CNPJ', { exact: false })).toHaveValue('12.345.678/0001-99');
  ...
  test('deve mostrar mensagem de sucesso ao salvar', async ({ page }) => {
    ...
    await page.getByLabel('Nome Completo', { exact: false }).fill('Usuario Teste');
  ...
  test('deve interagir com botões da aba de segurança', async ({ page }) => {
    ...
    const configurarBtn = page.getByRole('button', { name: 'Configurar' }).first();
